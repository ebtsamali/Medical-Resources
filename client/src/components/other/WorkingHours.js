...
const WorkingHours = ({weekDetails, setWeekDetails, disabled}) => {
            ...
            if (index >= startIndex && index <= endIndex) {
                return (<div key={index} className="day-container">
                    <BootstrapSwitchButton
                        ...
                        style="border"
                        size="sm"
                        disabled={(userRole === "user" || disabled) ? true : false}
                        onChange={handleDayStatusChange(index)}
                        width={65}/>
                    <TimePicker disabled={!day.isOpened || userRole === "user" || disabled ? true : false} className="time-picker" start="00:00" end="24:00" step={60}
